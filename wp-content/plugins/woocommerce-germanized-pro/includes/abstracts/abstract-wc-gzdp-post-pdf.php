...
class WC_GZDP_Post_PDF {
	...
	public function generate_pdf( $preview = false ) {
		...
		$tagvs = apply_filters( 'woocommerce_gzdp_pdf_vspace', array(
			'p' => array( 
				0 => array( 'h' => 1, 'n' => 1 ), 
				1 => array( 'h' => 0.05, 'n'=> 1 )
			),
		...
		do_action( "woocommerce_gzdp_after_generate_{$this->content_type}_config", $pdf, $this );
	...
	public function delete( $bypass_trash = false ) {
		update_post_meta( $this->id, '_deletion_allowed', '1' );
		...
		if ( $attachment )
			wp_delete_post( $attachment, $bypass_trash );
