...
class WC_GZDP_Invoice_Preview extends WC_GZDP_Invoice {
	...
	public function __construct() {
		...
		for ( $i = 0; $i < 30; $i++ ) {
			...
			if ( WC_GZDP_Dependencies::instance()->woocommerce_version_supports_crud() ) {
				...
				foreach( $item_data as $prop => $value ) {

					if ( in_array( $prop, array( 'line_total', 'line_subtotal', 'line_subtotal_tax' ) ) ) {
						$prop = substr( $prop, 5 );
					}

					$setter = "set_$prop";
					if ( ! is_null( $value ) && is_callable( array( $item, $setter ) ) ) {
						$reflection = new ReflectionMethod( $item, $setter );
	...
	public function get_totals( $tax_display = 'incl' ) {
		...
		$totals = parent::get_totals( $tax_display );
		$order_total = 0;

		foreach( $totals as $key => $total ) {
			if ( 'order_total' === $total[ 'key' ] ) {
				$order_total = $total[ 'value' ];
			}
		}

		foreach( $totals as $key => $total ) {
			if ( 'cart_subtotal' === $total[ 'key' ] ) {
				$totals[ $key ][ 'value' ] = $order_total;
			}
		}
